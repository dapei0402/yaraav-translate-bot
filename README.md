...
| `PASSWORD_SALT` | 密码哈希用的盐值，请改成你自己的随机字符串 |
...
- **密码安全**：用户密码经 SHA-256 + 盐 哈希后存储，不再明文保存。
- **频道级独立 API**：可为每个频道单独绑定专属 Gemini 密钥。
- **结构清晰**：单文件、模块分区明确，易于二次开发。

## 安全与鉴权

- **密码哈希存储**：注册、创建、审核、重置流程中，密码统一以 `SHA-256 + 盐` 哈希存储；所有者面板无法查看明文，只能「重置密码」。
- **平滑迁移**：历史遗留的明文密码在用户下次登录成功时会被自动升级为哈希。
- **配置盐值**：请在 `CONFIG.PASSWORD_SALT` 中改成你自己的随机字符串。

## API 密钥优先级

频道帖子的翻译按钮，按以下顺序选择要使用的 Gemini 密钥：

1. 该频道绑定的**专属 API**（在「频道设置 → 连接 / 更换该频道的 API」中设置）。
2. 频道所有者账户的**默认 API**。
3. 点击者自己登记的 API。
4. 都没有时回退到免费翻译接口。
...
- **盐值需自行配置**：`CONFIG.PASSWORD_SALT` 使用了默认占位值，正式使用前请改成你自己的随机字符串。
